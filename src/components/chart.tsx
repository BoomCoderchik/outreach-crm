...
function buildRoundedPath(pts: Array<{ x: number; y: number }>, radius: number): string {
...
export function Chart({
  ...
  return (
    ...
    >
      <div ref={root} onMouseMove={onMove} className="relative w-full aspect-[640/220] touch-none">
        <svg
          width="100%"
          height="100%"
        ...
        {(() => {
          ...
          return (
            <>
              ...
              >
                {labels?.[active.index] && (
                  <div className="font-medium text-foreground">{labels[active.index]}</div>
                )}
